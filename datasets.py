...
class TextDataset(data.Dataset):
    def __init__(self, data_dir, split='train',
        ...
        self.class_id = self.load_class_id(f'{data_dir}/{split}',self.number_example)
    ...
    def load_text_data(self, data_dir, split):
        ...
        #train_names = pickle.load(open(f'{data_dir}/train/filenames.pickle','rb'))
        #test_names = pickle.load(open(f'{data_dir}/test/filenames.pickle','rb'))
        filenames = pickle.load(open(os.path.join(data_dir, split, "filenames.pickle"),"rb")) #train_names if self.split=='train' else test_names
        x = pickle.load(open(filepath,'rb'))
        ixtoword, wordtoix = x[2],x[3]
        n_words = len(ixtoword)
        captions = x[0] if split=='train' else x[1]
        del x
